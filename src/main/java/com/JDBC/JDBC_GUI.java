        ...
        if (title != null) {
            try {
                ...
                if (resultSet.next()) {
                    ...
                    String updateQuery = "UPDATE books SET status = 'checked out', due_date = ? WHERE id = ?";
        ...
        if (title != null) {
            try {
                ...
                if (resultSet.next()) {
                    ...
                    int id = resultSet.getInt("id");
                    String updateQuery = "UPDATE books SET status = 'checked in', due_date = NULL WHERE id = ?";
                    PreparedStatement updateStatement = connection.prepareStatement(updateQuery);
    ...
    private void displayDatabase() {
        try {
            ...
            while (resultSet.next()) {
                ...
                stringBuilder.append("ID: ").append(id).append(", Title: ").append(title).append(", Author: ")
                        .append(author).append(", Barcode: ").append(barcode).append(", Status: ").append(status)
                        .append(", Due Date: ").append(dueDate).append("<br>");
            }
            outputLabel.setText("<html><body><div align='center'>Please make a selection</div><br>" + stringBuilder.toString() + "</body></html>");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
